...
const String _adultAgeConfirmedStorageKey = 'kumarket_adult_age_confirmed_v1';
...
class ApiService {
  ...
  String _stripTranslationArtifacts(String value) {
    ...
    text = text.replaceFirst(
      RegExp(r'^(перевод|translation)\s*:\s*', caseSensitive: false),
      '',
    );
    return text.trim();
  }
...
class AuthStore extends ChangeNotifier {
  ...
  Future<ActionResult> updateDeliveryAddress(DeliveryAddress address) async {
    ...
    try {
      ...
      _storage.write(_authStorageKey, next.toJson());
      notifyListeners();
      return const ActionResult(ok: true, message: 'Адрес сохранен.');
    } on ApiException catch (e) {
      return ActionResult(ok: false, message: e.message);
    } catch (_) {
      return const ActionResult(
  ...
  void _restore() {
    ...
    if (raw is List) {
      ...
      for (final item in raw) {
        if (item is Map<String, dynamic>) {
          final restored = FavoriteItem.fromJson(item);
          if (!item.containsKey('isAdult') &&
              !item.containsKey('is_adult') &&
              !item.containsKey('ageRestricted') &&
              !item.containsKey('age_restricted')) {
            parsed.add(
              FavoriteItem(
                ...
                reviewsCount: restored.reviewsCount,
                isAdult: true,
                ageRestricted: true,
              ),
            );
...
class CartStore extends ChangeNotifier {
  ...
  void _restore() {
    ...
    if (raw is List) {
      ...
      for (final item in raw) {
        if (item is Map<String, dynamic>) {
          final restored = CartItem.fromJson(item);
          if (!item.containsKey('isAdult') &&
              !item.containsKey('is_adult') &&
              !item.containsKey('ageRestricted') &&
              !item.containsKey('age_restricted')) {
...
class OrdersStore extends ChangeNotifier {
  ...
  Future<OrderCreateResult> createOrder(String paymentMethod) async {
    ...
    for (final item in _cart.items) {
      if ((item.variantId ?? '').trim().isEmpty) {
        return const OrderCreateResult(
          ok: false,
          message:
              'Перед оформлением выберите вариант товара (цвет/размер) для всех позиций в корзине.',
        );
      }
    }
    ...
    try {
      ...
      return OrderCreateResult(
        ok: true,
        message: 'Заказ отправлен в Django.',
        order: created,
      );
    } on ApiException catch (e) {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      _syncError = 'Не удалось обновить список заказов.';
      _restoreForCurrentUser();
    } finally {
      _isLoading = false;
      notifyListeners();
  ...
  CustomerOrder _mapOrder({
    ...
    for (final item in rawItems) {
      ...
      final ageRestricted = toSafeBool(
        item['ageRestricted'] ??
            item['age_restricted'] ??
            productMap?['ageRestricted'] ??
            productMap?['age_restricted'],
      );
...
class AdultAgeConfirmationStore extends ChangeNotifier {
  AdultAgeConfirmationStore(this._storage) {
    _restore();
  }

  final GetStorage _storage;
  bool _adultAgeConfirmed = false;

  bool get adultAgeConfirmed => _adultAgeConfirmed;
  bool get canShowAdultContent => _adultAgeConfirmed;

  void confirmAdultAge() {
    if (_adultAgeConfirmed) {
      return;
    }
    _adultAgeConfirmed = true;
    _persist();
    notifyListeners();
  }

  void resetAdultAgeConfirmation() {
    if (!_adultAgeConfirmed) {
      return;
    }
    _adultAgeConfirmed = false;
    _persist();
    notifyListeners();
  }

  void _restore() {
    final raw = _storage.read(_adultAgeConfirmedStorageKey);
    _adultAgeConfirmed = toSafeBool(raw);
  }

  void _persist() {
    _storage.write(_adultAgeConfirmedStorageKey, _adultAgeConfirmed);
  }
}
...
class AppStore {
  ...
  late final AdultAgeConfirmationStore adultAgeConfirmation =
      AdultAgeConfirmationStore(storage);
